...
async function updateArc(arcId: string, formData: FormData) {
  "use server";
  ...
  await prisma.arc.updateMany({
    where: { id: arcId, storyId: currentStory.id },
    data: {
      name,
      type,
  ...
  redirect(`/timeline/arcs/${arcId}`);
}

export default async function EditArcPage({
  params,
}: {
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        <div className="text-sm text-muted-foreground">
          Timeline / Arcs / {arc.name}
        </div>
        <h1 className="text-3xl font-semibold">Edit Arc</h1>
      </div>

      <form action={updateArc.bind(null, arc.id)} className="space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <label className="block text-sm font-medium">
            ...
            <select
              ...
              className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm"
              defaultValue={arc.startEpisodeNumber ?? ""}
            >
          ...
          <label className="block text-sm font-medium">
            ...
            <select
              ...
              className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm"
              defaultValue={arc.endEpisodeNumber ?? ""}
            >
              <option value="">Not set</option>
              {episodes.map((ep) => (
                <option key={ep.id} value={ep.episodeNumber}>
